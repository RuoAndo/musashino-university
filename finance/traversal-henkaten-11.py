...
# dynpは非常に重いので、dynpだけ上限を低くする
DYN_MAX_POINTS_FOR_CP = 500
SKIP_DYNP_IF_TOO_LONG = True
...
def format_elapsed(seconds):
    ...
    if minutes < 60:
        return f"{minutes:4d}分{sec:02d}秒"

    hours = minutes // 60
    minutes = minutes % 60
    return f"{hours:3d}時間{minutes:02d}分{sec:02d}秒"
...
def detect_change_points(returns, method, change_model, combo_index, total_combos):
    ...
    if method == "dynp" and SKIP_DYNP_IF_TOO_LONG and len(signal) > DYN_MAX_POINTS_FOR_CP:
        log_combo(
            combo_index,
            total_combos,
            method,
            change_model,
            f"dynpスキップ: len={len(signal)} > DYN_MAX_POINTS_FOR_CP={DYN_MAX_POINTS_FOR_CP}"
        )
        return []

    if len(signal) > max_points:
        idx = np.linspace(0, len(signal) - 1, max_points).astype(int)
    ...
    cp_elapsed = format_elapsed(time.time() - cp_start)
    ...
    if len(cps) == 0:
        log_combo(
            ...
            method,
            change_model,
            f"CP終了: 変化点なし, cp_time={cp_elapsed}"
        )
    else:
        log_combo(
            combo_index,
            ...
            method,
            change_model,
            f"CP終了: n_change_points={len(cps)}, sample={cps[:CP_PREVIEW_N]}, cp_time={cp_elapsed}"
        )
...
def process_one_file(path, index, total, method, change_model, combo_index, total_combos):
    ...
    try:
        ...
        log_combo(
            ...
            method,
            change_model,
            f"[{name}] AR終了: RMSE={rmse}, time={format_elapsed(time.time() - ar_start)}"
        )
        ...
        log_combo(
            ...
            method,
            change_model,
            f"[{name}] CP処理終了: current_file_cps={len(cps)}, time={format_elapsed(time.time() - cp_start)}"
        )
    ...
    summary["file_time"] = format_elapsed(file_elapsed)
...
def process_one_combination(
    ...
    log_combo(
        ...
        f"all_combos_cps={grand_total_cps}, time={format_elapsed(combo_elapsed)}"
    )
...
def main():
    ...
    log(f"MAX_POINTS_FOR_CP={MAX_POINTS_FOR_CP}")
    log(f"DYN_MAX_POINTS_FOR_CP={DYN_MAX_POINTS_FOR_CP}")
    log(f"SKIP_DYNP_IF_TOO_LONG={SKIP_DYNP_IF_TOO_LONG}")
    ...
    log(f"処理完了: total_time={format_elapsed(total_elapsed)}")
