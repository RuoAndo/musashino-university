...
CP_METHOD = "pelt"        # 変化点検出アルゴリズム
                           # "pelt"   : ペナルティ方式。自動的に変化点数を決める
                           # "binseg" : 変化点数をあらかじめ指定する方式

CHANGE_MODEL = "rbf"      # コスト関数
                           # "rbf" : 分散変化や非線形な変化も拾いやすい
                           # "l2"  : 平均値の変化を比較的素直に拾う
# CHANGE_MODEL = "l2"


# --- PELT 用 ---
CHANGE_PEN_BASE = 1.0     # ペナルティの強さ
                           # 小さいほど変化点が増える
                           # 大きいほど変化点が減る
                           # 例:
                           #   2.0 = やや保守的
...
# --- Binseg 用 ---
CHANGE_N_BKPS = 10        # Binseg の変化点数
                           # この値を増やすと、検出する変化点数も増える
                           # PELT を使う場合は通常あまり効かない


MIN_CHANGE_DISTANCE = 2   # 検出後に近すぎる変化点を間引くときの最小距離
                           # 小さいほど近接した変化点を残しやすい
                           # 大きいほど近い変化点をまとめやすい
                           # 頻度を高くしたいなら 1〜2 くらいが候補


CHANGE_MIN_SIZE = 3       # 1区間の最小長
                           # 小さいほど短い区間にも変化点を入れやすい
                           # 大きいほど細かい変化を抑える
                           # 頻度を高くしたいなら 2〜3 が候補


CHANGE_JUMP = 1           # 探索する候補点の間隔
                           # 1 なら全点を細かく探索
                           # 5 なら 5 点おきに粗く探索
                           # 小さいほど変化点を見つけやすいが計算は重くなる


MIN_RETURNS_FOR_CP = 10   # これ未満のデータ長では変化点検出をしない
                           # データが短すぎると誤検出しやすいための下限


MAX_POINTS_FOR_CP = 2000  # 長い系列はここまで間引く
                           # データ数が多すぎると計算が重くなるため上限を設定
                           # 小さすぎると細かい変化を落とすことがある


# ------------------------
# 頻度を高くしたいときの目安
# ------------------------
# まずは次の順で調整すると分かりやすいです
#
# 1. CHANGE_PEN_BASE を下げる
# 2. CHANGE_JUMP を下げる
# 3. CHANGE_MIN_SIZE を下げる
# 4. MIN_CHANGE_DISTANCE を下げる
#
# 例:
#CHANGE_PEN_BASE = 0.8
#CHANGE_MIN_SIZE = 3
...
# CHANGE_MIN_SIZE = 2
# CHANGE_JUMP = 1
# MIN_CHANGE_DISTANCE = 1
#
# ただし、頻度を上げすぎるとノイズまで変化点として拾いやすくなります。
...
OUTPUT_SUMMARY_CSV = Path.cwd() / f"change_point_summary_{CP_METHOD}_{CHANGE_MODEL}_{NOW_STR}.csv"
OUTPUT_PAIRS_CSV = Path.cwd() / f"change_point_pairs_{CP_METHOD}_{CHANGE_MODEL}_{NOW_STR}.csv"
