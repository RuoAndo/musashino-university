...
def detect_change_points(returns, method, change_model, combo_index, total_combos):
    ...
    log_combo(
        ...
        method,
        change_model,
        f"CP開始: len={n}, min_size={CHANGE_MIN_SIZE}, jump={CHANGE_JUMP}, n_bkps={n_bkps}"
    )
    ...
    if method == "pelt":
        ...
        raise ValueError(f"未対応のCP_METHODです: {method}")
    ...
    log_combo(
        ...
        method,
        change_model,
        f"CP終了: n_change_points={len(cps)}"
    )
...
def process_one_file(path, index, total, method, change_model, combo_index, total_combos):
    ...
    try:
        ...
        log_combo(
            ...
            method,
            change_model,
            f"[{name}] 読込完了: rows={len(df)}, return_rows={len(returns)}"
        )
        ...
        for cp in cps:
            ...
            before = returns[max(0, cp - 5):cp]
            after = returns[cp:min(len(returns), cp + 5)]

            if len(before) > 0 and len(after) > 0:
                strength = float(abs(np.mean(after) - np.mean(before)))
            else:
                strength = np.nan
        ...
        log_combo(
            ...
            method,
            change_model,
            f"[{name}] CP終了: n={len(cps)}, time={time.time() - cp_start:.2f}s"
        )
    ...
    log_combo(
        ...
        method,
        change_model,
        f"--- {name} 処理終了: total_time={time.time() - start:.2f}s, progress={progress:.1f}% ---"
    )
    ...
    log_combo(
        ...
        method,
        change_model,
        f"[{name}] summary: rows={summary['rows']}, return_rows={summary['return_rows']}, "
        f"rmse={summary['rmse']}, n_change_points={summary['n_change_points']}, status={summary['status']}"
    )
...
def process_one_combination(files, method, change_model, combo_index, total_combos):
    ...
    log_combo(
        ...
        method,
        change_model,
        f"DataFrame化完了: summary_rows={len(summary_df)}, pairs_rows={len(pairs_df)}"
    )
    ...
    summary_path = (
        ...
        f"{method}_{change_model}_{timestamp}.csv"
    )
    ...
    log_combo(
        ...
        method,
        change_model,
        f"組み合わせ処理終了: time={time.time() - combo_start:.2f}s"
    )
...
def main():
    ...
    for combo_index, (method, change_model) in enumerate(combinations, start=1):
        try:
            summary_df, pairs_df = process_one_combination(
                files=files,
                method=method,
                change_model=change_model,
                combo_index=combo_index,
    ...
    master_summary_path = (
        f"{OUTPUT_PREFIX}_summary_ALL_METHODS_ALL_MODELS_{timestamp}.csv"
    )

    master_pairs_path = (
        f"{OUTPUT_PREFIX}_pairs_ALL_METHODS_ALL_MODELS_{timestamp}.csv"
    )
    ...
    if len(master_summary_df) > 0:
        ...
        try:
            ...
            agg_path = (
                f"{OUTPUT_PREFIX}_summary_AGG_ALL_METHODS_ALL_MODELS_{timestamp}.csv"
            )
