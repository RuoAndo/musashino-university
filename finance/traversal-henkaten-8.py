...
CP_METHODS = ["pelt", "binseg", "bottomup", "window"]
CHANGE_MODELS = ["l1", "l2", "rbf"]
...
def detect_change_points(returns, method, change_model, combo_index, total_combos):
    ...
    if len(returns) < MIN_RETURNS_FOR_CP:
        log_combo(
            ...
            method,
            change_model,
            f"変化点検知スキップ: returns={len(returns)} < MIN_RETURNS_FOR_CP={MIN_RETURNS_FOR_CP}"
        )
        return []
    ...
    if len(cps) == 0:
        ...
        log_combo(
            ...
            method,
            change_model,
            f"CP終了: n_change_points={len(cps)}, cp_index_sample={cps[:CP_PREVIEW_N]}"
        )
...
def log_change_point_details(
    ...
    for row in preview_rows:
        log_combo(
            ...
            method,
            change_model,
            f"[{name}] CP index={row['cp_index']}, datetime={row['datetime']}, strength={row['strength']}"
        )
...
def process_one_file(path, index, total, method, change_model, combo_index, total_combos):
    ...
    try:
        ...
        log_combo(
            ...
            method,
            change_model,
            f"[{name}] CP処理終了: n={len(cps)}, time={time.time() - cp_start:.2f}s"
        )
