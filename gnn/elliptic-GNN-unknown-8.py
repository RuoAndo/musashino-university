...
外れ値トランザクションをソート出力する完全版。

※プロット表示なし
※time step 集計は横にはみ出さない縦表示
"""
...
OUTPUT_CSV_ANY = "unknown_outliers_any_3type_sorted.csv"

OUTPUT_TX_OUTLIER = "transactions_sorted_by_outlier.csv"
OUTPUT_TX_TXID = "transactions_sorted_by_txid.csv"
OUTPUT_TX_TIME = "transactions_sorted_by_time_txid.csv"
OUTPUT_TX_OUTLIERS_ONLY = "transactions_outliers_only_sorted.csv"
...
# 見やすい縦表示
# ==========================
def print_records_vertical(df: pd.DataFrame, title: str, top_k: int = 30):
    print(f"\n=== {title} ===")
        ...
        for col in show_df.columns:
            ...
            if isinstance(v, float):
                if abs(v) < 0.0001 and v != 0:
                    print(f"{col:38s}: {v:.10e}")
                else:
                    print(f"{col:38s}: {v:.6f}")
            else:
                print(f"{col:38s}: {v}")
...
# ランキング表示
# ==========================
def print_transaction_rankings(df: pd.DataFrame, top_k: int = 30):
    with PhaseTimer("トランザクション別ソート表示"):
        view_cols = [
            "txId",
            "node_index",
            "time_step",
        ...
        if "three_type_max_outlier_score" in df.columns:
            ...
            print_records_vertical(
                df_rank,
                f"トランザクション外れ値スコア順トップ{top_k}",
                top_k=top_k
            )

        df_txid = df.sort_values("txId")[view_cols]

        print_records_vertical(
            df_txid,
            f"トランザクション txId 昇順トップ{top_k}",
            top_k=top_k
        )

        df_time = df.sort_values(["time_step", "txId"])[view_cols]

        print_records_vertical(
            df_time,
            f"トランザクション time_step → txId 順トップ{top_k}",
            top_k=top_k
        )

        if "is_any_3type_outlier" in df.columns:
            df_out = df[df["is_any_3type_outlier"] == True].copy()
...
def print_sorted_outlier_rankings(df: pd.DataFrame, top_k: int = 30):
    with PhaseTimer("3タイプ別 外れ値ランキング表示"):
        ...
        if "degree_outlier_score" in df.columns:
            ...
            print_records_vertical(
                df_rank,
                f"1) 大量接続ノード: degree が高い外れ値トップ{top_k}",
                top_k=top_k
            )
        ...
        if "betweenness_centrality_outlier_score" in df.columns:
            ...
            print_records_vertical(
                df_rank,
                f"2) 資金流路の橋渡しノード: betweenness が高い外れ値トップ{top_k}",
                top_k=top_k
            )
        ...
        if "pagerank_outlier_score" in df.columns:
            ...
            print_records_vertical(
                df_rank,
                f"3) 重要ノード群に接続する目立つノード: PageRank が高い外れ値トップ{top_k}",
                top_k=top_k
            )
...
def analyze_unknown_nodes_3type_outliers(
    ...
    with PhaseTimer("unknown ノード 3タイプ外れ値分析"):
        ...
        log("unknown トランザクションごとの中心性テーブル作成中...")
        ...
        for i, n in enumerate(unknown_nodes, start=1):
            ...
            if i % 10000 == 0:
                log(f"unknown トランザクション集計進捗: {i}/{n_unknown}")

        df_unknown = pd.DataFrame(rows)

        log(f"unknown トランザクション中心性テーブル完成: shape={df_unknown.shape}")
        ...
        print_transaction_rankings(
            df_unknown,
            top_k=PRINT_TRANSACTION_TOP_K
        )

        print("\n=== unknown トランザクション中心性・外れ値スコアの基本統計 ===")
        stat_cols = [
            c for c in [
                "degree",
                "degree_centrality",
        ...
        print(df_unknown[stat_cols].describe().to_string())
...
def summarize_unknown_by_time_step(df_unknown: pd.DataFrame):
    ...
    with PhaseTimer("time step ごとの unknown 外れ値集計"):
        ...
        for _, row in summary.iterrows():
            ...
            print(f"unknown_count                 : {int(row['unknown_count'])}")
            print(f"avg_degree                    : {row['avg_degree']:.6f}")

            if "degree_outlier_count" in summary.columns:
                print(f"degree_outlier_count          : {int(row['degree_outlier_count'])}")

            if "betweenness_outlier_count" in summary.columns:
                print(f"betweenness_outlier_count     : {int(row['betweenness_outlier_count'])}")

            if "pagerank_outlier_count" in summary.columns:
                print(f"pagerank_outlier_count        : {int(row['pagerank_outlier_count'])}")

            if "any_3type_outlier_count" in summary.columns:
                print(f"any_3type_outlier_count       : {int(row['any_3type_outlier_count'])}")

            if "max_outlier_score" in summary.columns:
                print(f"max_outlier_score             : {row['max_outlier_score']:.6f}")

            if "avg_outlier_score" in summary.columns:
                print(f"avg_outlier_score             : {row['avg_outlier_score']:.6f}")

            if "avg_pagerank" in summary.columns:
                print(f"avg_pagerank                  : {row['avg_pagerank']:.10f}")

            if "avg_betweenness" in summary.columns:
                print(f"avg_betweenness               : {row['avg_betweenness']:.10f}")

        output_summary = "unknown_outlier_summary_by_time_step.csv"
        summary.to_csv(output_summary, index=False, encoding="utf-8-sig")
        log(f"time step 集計CSV保存: {output_summary}")
...
def save_outputs(df_unknown: pd.DataFrame):
    ...
    with PhaseTimer("CSV保存"):
        ...
        df_all_sorted = df_unknown.sort_values(
            sort_col,
            ascending=False
        )
        ...
        if "degree_is_outlier" in df_unknown.columns:
            df_degree = (
                df_unknown[df_unknown["degree_is_outlier"] == True]
                .sort_values(
                    ["degree_outlier_score", "degree"],
                    ascending=[False, False]
                )
            ...
            log(f"degree外れ値CSV保存: {OUTPUT_CSV_DEGREE} | rows={len(df_degree)}")

        if "betweenness_centrality_is_outlier" in df_unknown.columns:
            df_bet = (
                df_unknown[df_unknown["betweenness_centrality_is_outlier"] == True]
                .sort_values(
                    [
                        "betweenness_centrality_outlier_score",
                        "betweenness_centrality",
            ...
            log(f"betweenness外れ値CSV保存: {OUTPUT_CSV_BETWEENNESS} | rows={len(df_bet)}")

        if "pagerank_is_outlier" in df_unknown.columns:
            df_pr = (
                df_unknown[df_unknown["pagerank_is_outlier"] == True]
                .sort_values(
                    ["pagerank_outlier_score", "pagerank", "degree"],
                    ascending=[False, False, False]
                )
            ...
            log(f"pagerank外れ値CSV保存: {OUTPUT_CSV_PAGERANK} | rows={len(df_pr)}")

        if "is_any_3type_outlier" in df_unknown.columns:
            df_any = (
                df_unknown[df_unknown["is_any_3type_outlier"] == True]
                .sort_values(
                    ["three_type_max_outlier_score", "three_type_sum_outlier_score"],
                    ascending=[False, False]
                )
            ...
            log(f"3タイプいずれかの外れ値CSV保存: {OUTPUT_CSV_ANY} | rows={len(df_any)}")

        save_transaction_sorted_outputs(df_unknown)


def save_transaction_sorted_outputs(df_unknown: pd.DataFrame):
    with PhaseTimer("トランザクション別ソートCSV保存"):
        if "three_type_max_outlier_score" in df_unknown.columns:
            df_tx_outlier = df_unknown.sort_values(
                ["three_type_max_outlier_score", "three_type_sum_outlier_score"],
                ascending=[False, False]
            )

            df_tx_outlier.to_csv(
                OUTPUT_TX_OUTLIER,
                index=False,
                encoding="utf-8-sig"
            )
            log(f"トランザクション外れ値スコア順CSV保存: {OUTPUT_TX_OUTLIER}")

        df_txid = df_unknown.sort_values("txId")
        ...
        log(f"トランザクションtxId順CSV保存: {OUTPUT_TX_TXID}")

        df_time = df_unknown.sort_values(["time_step", "txId"])
        ...
        log(f"トランザクションtime_step→txId順CSV保存: {OUTPUT_TX_TIME}")

        if "is_any_3type_outlier" in df_unknown.columns:
            df_outliers_only = (
                df_unknown[df_unknown["is_any_3type_outlier"] == True]
                .sort_values(
                    ["three_type_max_outlier_score", "three_type_sum_outlier_score", "txId"],
                    ascending=[False, False, True]
                )
            )

            df_outliers_only.to_csv(
                OUTPUT_TX_OUTLIERS_ONLY,
                index=False,
                encoding="utf-8-sig"
            )
            log(
                f"外れ値トランザクションのみCSV保存: "
                f"{OUTPUT_TX_OUTLIERS_ONLY} | rows={len(df_outliers_only)}"
            )
